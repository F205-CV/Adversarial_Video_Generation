...
class GeneratorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('generator'):
            ...
            self.scale_inputs_train = [] # the input images (except gt) at each scale
                ...
                with tf.name_scope('scale_' + str(scale_num)):
                    ...
                    with tf.name_scope('calculation'):
                        def calculate(height, width, inputs, gts, last_gen_frames):
                            ...
                            scaled_inputs = tf.image.resize_images(inputs, [scale_height, scale_width])
                            ...
                            if scale_num > 0:
                                ...
                                inputs = tf.concat([scaled_inputs, last_gen_frames], 3)
                            ...
                            return preds, scale_gts, scaled_inputs
                        ...
                        # calculate
                        train_preds, train_gts, train_inputs = calculate(
                                                           self.height_train,
                                                           self.width_train,
                        ...
                        self.scale_inputs_train.append(train_inputs)
                        ...
                        # calculate
                        test_preds, test_gts, _ = calculate(self.height_test,
                                                         self.width_test,
    ...
    def train_step(self, batch, discriminator = None):
        ...
        if c.ADVERSARIAL:
            ...
            scale_preds, scale_inputs = self.sess.run([self.scale_preds_train,
                            self.scale_inputs_train], feed_dict = feed_dict)
            ...
            for scale_num, inputs_patch in enumerate(scale_inputs):
                shape = np.shape(inputs_patch)
                # print 'inputs_patch:', shape
                scaled_full_frames = np.empty([batch_size * frame_len, shape[1], shape[2], 3])

                for i, img in enumerate(inputs_patch):
                    # print 'img:', np.shape(img)
                    base = np.stack(np.split(img, c.HIST_LEN, 2))
                    scaled_full_frames[i * frame_len : (i + 1) * frame_len] = \
                                np.concatenate([base, scale_preds[scale_num][i, None]])

                d_feed_dict[discriminator.scale_nets[scale_num].input_frames] = scaled_full_frames
